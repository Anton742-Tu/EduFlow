...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        delete_course_permission = Permission.objects.get(
            codename='delete_course',
            content_type=course_content_type
        )
        delete_lesson_permission = Permission.objects.get(
            codename='delete_lesson',
            content_type=lesson_content_type
        )
        add_course_permission = Permission.objects.get(
            codename='add_course',
            content_type=course_content_type
        )
        add_lesson_permission = Permission.objects.get(
            codename='add_lesson',
            content_type=lesson_content_type
        )
        ...
        self.stdout.write(self.style.SUCCESS(
            f'Группе модераторов назначены права: просмотр и изменение курсов/уроков'
        ))
        self.stdout.write(self.style.WARNING(
            f'Группе модераторов запрещены: создание и удаление курсов/уроков'
        ))
