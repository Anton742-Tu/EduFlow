...
class PaymentsFilter(filters.FilterSet):
    ...
    payment_date_after = filters.DateTimeFilter(
        field_name="payment_date", lookup_expr="gte", label="Дата оплаты после"
    )
    payment_date_before = filters.DateTimeFilter(field_name="payment_date", lookup_expr="lte", label="Дата оплаты до")

    class Meta:
        model = Payments
        fields = ["course", "lesson", "payment_method", "payment_date_after", "payment_date_before"]
